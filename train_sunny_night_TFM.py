    ...
    for idx, (night, sunny, night_name, sunny_name) in enumerate(loop):
        ...
        if epoch % 10 == 0:
            if idx % 200 == 0:
                ...
                path = "/media/arvc/DATOS/TFG Carlos/Data/saved_images_TFM/SunnyNight"
                # crea la carpeta si no existe
                import os
                try:
                    os.makedirs(path)
                except OSError:
                    path = "saved_images_TFM/SunnyNight"
...
def main():
    ...
    plt.figure("Train")
    plt.plot(list_epoch,list_cycle_loss_sunny_mean, color='r', label = "Cycle Loss Sunny TFM")
    plt.plot(list_epoch,list_cycle_loss_night_mean, color='b', label = "Cycle Loss Night TFM")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig(config.MODEL_DIR_SN + "/CycleLossTrainSN_TFM.jpg", bbox_inches = 'tight')

    plt.figure("Validation")
    plt.plot(list_epoch, list_cycle_loss_sunny_mean_val, color='r', label="Cycle Loss Sunny val TFM")
    plt.plot(list_epoch, list_cycle_loss_night_mean_val, color='b', label="Cycle Loss Night val TFM")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig(config.MODEL_DIR_SN + "/CycleLossValSN_TFM.jpg", bbox_inches = 'tight')

    plt.figure("Discriminator Loss")
    plt.plot(list_epoch, list_D_loss_mean, color='r', label="D_loss train TFM")
    plt.plot(list_epoch, list_D_loss_mean_val, color='b', label="D_loss val TFM")
    plt.xlabel("Epoch")
    plt.ylabel("D_Loss")
    plt.legend()
    plt.savefig(config.MODEL_DIR_SN + "/D_loss_SN_TFM.jpg", bbox_inches='tight')
    plt.show()
